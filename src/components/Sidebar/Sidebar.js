...
function Sidebar({ toggleSidenav, sidenavOpen, routes, logo, rtlActive }) {
  ...
  const [collapsedViews, setCollapsedViews] = React.useState({});
  ...
  const getCollapseStates = (routes) => {
    ...
    routes.map((prop, key) => {
      if (prop.collapse) {
        ...
        setCollapsedViews(initialState); // Set the collapsedViews state
      }
      return null;
    });
    return initialState;
  ...
  const createLinks = (routes) => {
    return routes.map((props, key) => {
      ...
      if (props.collapse) {
        const isCollapsed = collapsedViews[props.state];
          ...
          <NavItem key={key}>
            <NavLink
              ...
              className={classnames({
                active: getCollapseInitialState(props.views),
              })}
              onClick={(e) => {
                e.preventDefault();
                setState(st);
